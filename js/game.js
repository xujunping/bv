...
game.prototype={
	...
	move2:function(){
		for (var i = 0; i < that.span.length; i++) {
			...
			if(tops>that.height){
				...
				var dieImg=document.getElementsByClassName('message')[0].getElementsByTagName('img')[1];
	...
	createEle:function(num){
		...
		for (var i = 0; i < arr.length; i++) {
			...
			span.style.cssText="position:absolute;height:80px;width:80px;font-size:30px;font-family:Arial;color:rgba(255,255,255,0);top:"+(-300*Math.random()-85)+"px;left:"+(50+Math.random()*(this.width-100))+"px;background: url(img/"+arr[i]+".png) no-repeat center center;background-size:80px 80px;";
